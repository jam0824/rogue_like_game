...
describe("floorProgression", () => {
  it("clampFloor は 1..10 の範囲に丸める", () => {
    ...
    expect(clampFloor(10)).toBe(MAX_FLOOR);
  ...
  it("resolveDungeonIdForFloor は floor を dungeon_id_XX に変換する", () => {
    ...
    expect(resolveDungeonIdForFloor(10)).toBe("dungeon_id_10");
    expect(resolveDungeonIdForFloor(99)).toBe("dungeon_id_10");
  });
  ...
  it("resolveFloorFromDungeonId は dungeon_id_XX から floor を復元し範囲外を clamp する", () => {
    ...
    expect(resolveFloorFromDungeonId("dungeon_id_20")).toBe(10);
