...
| ARC | 霊性/術式：状態付与・DoT/デバフ・相性（リアクション）側の伸びしろ | AilmentApply, DoT/デバフ係数, ReactionBonus |
...
#### レベルアップの扱い（推奨：ステータスポイント方式）
ラン内LVは、派生値式そのものを変えるのではなく、**`StatRun` / `RunBonus` を増やすための仕組み**として扱う。

- レベルアップごとに `RunPoint += 1`
- `RunPoint` は任意の一次ステータスへ割り振れる：
  - `+1 VIT` / `+1 FOR` / `+1 AGI` / `+1 POW` / `+1 TEC` / `+1 ARC`
- 割り振り結果は `StatRun` に反映される（例：VITに2回振ったら `StatRun.VIT += 2`）

#### 設計上の注意（推奨）
- RunLevelで **武器の攻撃周期（クールダウン）**や**複製（本数増加）**へ直接影響させない（ビルド領域を壊しやすいため）。
- クリティカル率・移動速度・被ダメ軽減には上限がある前提で運用する（暴れ防止）。
...
CC連打防止（推奨）：
- 凍結終了後：`FreezeImmunity = 3s`
- 麻痺終了後：`ParalyzeImmunity = 3s`
