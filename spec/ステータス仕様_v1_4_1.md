...
- バージョン: v1.4.1  
- 更新日: 2026-02-10（player_state例の武器フィールド補完）
...
- `equipped_weapons[]`：装備中武器
  - `weapon`：武器インスタンス（詳細は「武器システム仕様」）
    - **UI/表示に必要なメタ（`name_key` / `description_key` / `weapon_file_name` / `width` / `height`）も、武器インスタンスの必須フィールドとして保存する**
